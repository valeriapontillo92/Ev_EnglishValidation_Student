      ...
      if (flag == 1) { // Preleva tutte le richieste
        try {
          ...
          sql = "SELECT r.id_request, r.certificate_serial, r.level, r.release_date,"
              ...
              + ")";
          ResultSet r = stmtSelect.executeQuery(sql); //ho inserito sql che è la query invece di 'null'
          if (r.wasNull()) {
        ...
        try {
          ...
          stmt = conn.prepareStatement(sql);
          stmt.setInt(2, idRequest);  //ho invertito l'assegnamento allo statement
